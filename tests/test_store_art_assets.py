...
def test_the_asset_has_the_exact_dimensions_the_store_demands(path: Path) -> None:
    assert Image.open(path).size == EXPECTED[path]
...
def test_committed_assets_match_the_generator() -> None:
    """The drift guard. Compares decoded pixels, never bytes.

    PNG encoding is not reproducible across platforms -- a different zlib or Pillow
    build re-encodes identical pixels into a different stream -- and byte comparison is
    what once turned the Windows and macOS release legs red on correct assets.
    """
