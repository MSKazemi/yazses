...
def test_registry_contains_only_english_in_behavior_preserving_refactor():
    assert registered_languages() == ("en",)
    assert get_grammar("en") is ENGLISH_GRAMMAR
    assert get_grammar("en-US") is ENGLISH_GRAMMAR


def test_unknown_language_is_not_silently_mapped_to_english():
