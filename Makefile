...
# Redraw every shipped icon from the one brand-mark renderer: the Windows .ico, the
# macOS .icns, the four .deb hicolor PNGs and the Snap Store icon. They are committed
# artifacts (PyInstaller and Inno Setup need them in a plain checkout, and Pillow is not
# a runtime dependency off Windows), so this is run when the mark changes. It exists
# because the two Linux icons were shipped by the build scripts and regenerated by
# nothing: the .deb PNGs had drifted to an older render, and the snap icon was a
# different logo altogether. `--check` reports drift without writing; a test fails on it.
icons:
