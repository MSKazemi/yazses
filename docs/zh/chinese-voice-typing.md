...
当 `[commands] enabled = true`（默认值）且 `[commands] language = "auto"` 时，
命令语法会跟随 `[stt] language`。普通话模式目前只接受严格的整句命令，并同时覆盖简体与繁体：
撤销、保存、复制／剪切／粘贴、有限范围的删词／删行与选择、回车／Tab／Escape／退格，
以及行、页面、光标和“跳到函数／类／文件”这类明确的导航命令。

```toml
[commands]
language = "auto"  # 跟随 [stt] language；也可显式设为 "en" 或 "zh"
```

这**还不是英语命令集的完整中文等价版**。P1 故意不提供英语 `run ...` 对应的终端执行规则，
也不提供开放式重构／创建命令。普通听写键下，未匹配的中文仍按普通文字输入；专用命令键下，
未匹配内容保持未匹配，不会执行。中文 Tier-1 也不会继续交给英语 SLM 模糊分类器，因此普通中文
句子不会因为包含“保存”“删除”等词就被提升成英语命令。
