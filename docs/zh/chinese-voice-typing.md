---
title: "中文离线语音输入 —— 免费开源的电脑语音转文字工具"
description: "不联网、不注册、不订阅，直接用说话的方式把中文输入到任何应用。支持 Windows、Linux 与 macOS 的离线普通话语音输入，简体或繁体由你指定，而不是由模型随机决定。"
alternates:
...
**一句话总结：** YazSes 是一款免费、开源的听写软件，可以把中文直接输入到电脑上的任何应用，
并且在拔掉网线的情况下照样工作。你按住一个键，说话，松开，文字就出现在光标所在的位置。
默认不上传任何内容，不需要注册账号，也没有按分钟计费。
