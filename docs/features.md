...
description: All 146 YazSes capabilities, what each does, and how to turn it on.
---
...
YazSes ships **146 capabilities**. All but the core are **off by default** — turn one on with `yazses features enable <toggle>` and apply with `yazses restart`. Browse them interactively with `yazses features` (grouped) or `yazses features info <toggle>` (one card).
...
- [Accessibility & input modalities](#accessibility--input-modalities) (21)
