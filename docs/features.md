...
description: All 142 YazSes capabilities, what each does, and how to turn it on.
---
...
YazSes ships **142 capabilities**. All but the core are **off by default** — turn one on with `yazses features enable <toggle>` and apply with `yazses restart`. Browse them interactively with `yazses features` (grouped) or `yazses features info <toggle>` (one card).
