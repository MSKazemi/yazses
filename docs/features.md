...
description: All 148 YazSes capabilities, what each does, and how to turn it on.
---
...
YazSes ships **148 capabilities**. All but the core are **off by default** — turn one on with `yazses features enable <toggle>` and apply with `yazses restart`. Browse them interactively with `yazses features` (grouped) or `yazses features info <toggle>` (one card).
...
- [Accessibility & input modalities](#accessibility--input-modalities) (23)
...
- **Toggle:** `windowctl`  ·  **Tier:** optional  ·  **Config:** `[windowctl] — focus and arrange windows`
- **What it does:** Say 'focus the browser' to raise a window by name, or 'move window left half', 'maximize', 'workspace 3' to arrange them. X11 only (Wayland does not let one app focus or move another's window). Off by default.
...
### Face-Gesture Switch (camera)

- **Toggle:** `facegesture`  ·  **Tier:** experimental — not advised yet  ·  **Config:** `[facegesture] — hold a face, not a key`
- **What it does:** Opens the mic while you hold a facial movement (jaw open, brows raised) instead of a key — switch access for people who can speak but cannot press one. Needs a webcam; experimental. Off by default.
- **Use when:** When you cannot hold a key — a facial movement becomes the hotkey, using only the webcam.
- **Example:** Open your jaw to start dictating, close it to stop — no key at all.
- **Download:** ~220 MB
- **Activate:** `yazses features enable facegesture` then `yazses restart`
