...
## Verify build provenance (works even while binaries are unsigned)

Every release artifact — the `.deb`, the `.dmg`, the `.exe` and the PyPI wheel —
carries a **build provenance attestation**: a signed statement that *this exact
file was produced by this workflow, from this commit, in this repository*. It is
generated by GitHub's OIDC identity during the release run, so there is no private
key for anyone to leak or steal.

```bash
gh attestation verify yazses_2.18.2_amd64.deb --repo MSKazemi/yazses
```

A pass tells you the file came out of this repository's CI and has not been altered
since. That is a different guarantee from a code signature, and for now a stronger
one here: **the binaries are not yet code-signed**, so provenance is the only
cryptographic link between a download and this source tree.

For the PyPI wheel, attestations are [PEP 740](https://peps.python.org/pep-0740/)
and are attached by the same trusted-publishing identity that uploads it — pip and
`uv` can check them without any extra step from you.

> **Not yet proven.** These attestations were added on 2026-08-14 and the first
> release to carry them has not been cut. If `gh attestation verify` fails on an
> artifact older than that, the artifact predates the change rather than being
> tampered with — check the release date before assuming the worst.
