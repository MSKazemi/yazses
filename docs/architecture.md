...
It also prints what a capability **costs**. `system/depsize.py` reads a committed
table (`system/feature_sizes.json`, regenerated by `scripts/gen-feature-sizes.py`)
and reports the download size before anything is fetched — the *marginal* size, so a
feature whose packages you already have reports as free rather than as its full
