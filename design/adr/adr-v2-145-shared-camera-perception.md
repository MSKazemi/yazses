# ADR-v2-145 — Shared camera perception source for gaze, head pose and face switches
...
3. **Face-gesture switches** are live. ADR-v2-135 shipped `facegesture/detector.py` (a pure
   hysteretic hold detector over one blendshape score) and `facegesture/backend.py`, which opens
   its *own* `cv2.VideoCapture` and its *own* FaceLandmarker to feed it.
...
Two of them already do open their own `cv2.VideoCapture` and their own landmarker — `gaze` and
`facegesture` — with nothing arbitrating between them, so this is a defect to repair rather than
only a future one to avoid. Where each feature owns its own camera, YazSes gets:
