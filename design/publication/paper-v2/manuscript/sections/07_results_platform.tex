...
\texttt{tiny.en} shows a 0.49-point spread. This is sufficient to reject the assumption that the
third decimal place of a WER measured on one CPU should automatically be transported to another,
but it is not evidence that every checkpoint is meaningfully sensitive to platform.

\subsection{Installability is a different portability layer}

The companion dependency-resolution experiment covers 92 installation target combinations, of
which 84 resolve in the archived result. Resolution failures expose platform/package constraints
that are distinct from recognition accuracy.
