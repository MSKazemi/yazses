...
The engine matrix exposed the disagreement. It was run twice on the same instance over the same
200 \texttt{test-other} utterances, and the two runs put \texttt{large-v3} 2.83 \wer{} points
apart: 4.86\% and 7.69\%, the latter with 184 insertions. At that time a repeated benchmark
overwrote its own output file, so the first run survives only as the console log
\texttt{paper/results/probes/logs/x86b-other\_wer.log} while the second is the committed
artifact. Every other engine in the matrix agreed between the two runs to within 0.16 points.
The near-loss of the contradicting run is what motivated the non-destructive result
replacement described in Section~\ref{sec:method}, and it is why this one number is cited to a
log rather than to an artifact.

We then ran a dedicated four-repeat probe over the same utterances. Those controlled repeats
produced 6.53\%, 6.07\%, 5.46\%, and 6.61\% \wer{}.
...
The 7.69\% matrix run extends the campaign-wide observed insertion count to 184, but we keep it
separate from the controlled four-repeat range rather than silently pooling the two experimental
contexts.

A third context adds three further default-configuration decodes of the same subset: the
baseline arm of the decoder-intervention probe below, at 6.07\%, 4.78\%, and 4.86\%. Across all
three contexts the archive holds eight decodes of one nominally fixed
configuration on one subset, spanning 4.78\% to 7.69\%. They were taken on different days under
different \YazSes{} and NumPy versions, so we do not pool them into one distribution; we use
only the four-repeat probe, which held everything else fixed, for the error decomposition above.
