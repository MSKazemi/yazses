...
unchanged across the repeated runs used in the error decomposition. A dedicated four-repeat probe localized the movement to insertion count, which ranged from
101 to 144 while substitutions, deletions, and hits remained fixed. The engine-matrix run that
triggered the investigation reached 184 insertions and 7.69\% \wer{}, and disagreed with its own
repeat on the same instance by 2.83 \wer{} points. For an offline benchmark, this appears as
movement in aggregate \wer{}. For interactive dictation, it is a different class of failure: a
fluent
...
test split, with 75.21\% mean \der{} and a mean signed speaker-count error of +155.19.
A meeting-specific threshold reduced mean \der{} to 26.71\%. Capping clustering at four
speakers made the estimated count exact on 15 of 16 recordings---the reference split contains
fifteen four-speaker recordings and one three-speaker recording---but did not establish an
improvement in \der{} at the recalibrated threshold.
