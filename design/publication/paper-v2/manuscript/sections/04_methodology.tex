...
The onset experiment uses exact two-sided McNemar tests on paired first-word outcomes. Each
non-zero lead-in is compared against the zero-lead-in cell of its own clipping arm, which gives
sixteen distinct comparisons. The whole grid was run end to end twice over the same utterances
by a decoder that is deterministic on identical input, so the second pass supplies replicates
rather than sixteen further hypotheses: the multiplicity family is the sixteen comparisons
asked, and the Bonferroni threshold is $0.05/16 = 0.003125$. Replication is reported separately,
as whether a comparison held in both passes.
