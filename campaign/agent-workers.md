...
Your provider account and quota are yours. YazSes does not collect, share, reimburse, or manage
provider credentials or usage.

Never put an agent API key, Google token, GitHub token, provider secret, or billing credential in
an issue, pull request, task manifest, committed config file, screenshot, or project log.

Provider pricing, quotas, and account rules can change. Check the provider's current official
documentation before enabling paid or metered use.
