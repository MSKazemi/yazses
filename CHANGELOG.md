...
`yazses[diarization]==<version>` and the PyPI upload happens in a workflow the
same tag starts. On v2.36.0 that wait printed *"the index pip reads serves
yazses 2.36.0"* and the build failed **fifteen seconds later** listing 2.35.0 as
the newest version.

This is the v2.24.0 failure one layer down. Then, the guard polled
`https://pypi.org/pypi/yazses/<v>/json`, saw 200, and the build failed sixteen
seconds later — so it was moved to the PEP 503 simple index, "what pip resolves
against". But `curl` with no `Accept` header gets the **HTML** representation of
that index and pip asks for the **PEP 691 JSON** one, which is cached
separately. The guard was again reading a neighbour of what pip reads.

There is no third representation to move to, and that is the point: every check
that is merely *near* pip can be right while pip is wrong. The wait now runs
`pip download --no-deps`, the resolver that actually fails. The objection
recorded against doing that — a runner without pip cannot resolve anything and
reports it exactly like an unpublished release, a false negative that blocks
every build — is answered rather than avoided: pip is proved to exist first,
with its own distinct error. `--no-cache-dir` is there so an early negative
cannot be re-served for the rest of the loop.

The race is real rather than theoretical: v2.34.0 and v2.35.0 won it and
v2.36.0 lost it, so the container simply did not publish until the job was
re-run by hand.

### Fixed — every release published ARM-less Windows manifests and then repaired them by hand

`publish-channels.yml` waited for two filenames —
`YazSes-<v>-windows-x64.exe` and `YazSes-<v>-macos-arm64.dmg` — and then ran the
manifest generator. `scripts/refresh-package-manifests.py` also consumes
`YazSes-<v>-windows-arm64.exe` **when it is present**, and treats it as optional
on purpose: that leg of `build-windows` is `continue-on-error`, so a release may
legitimately ship without one.

From inside the wait, "legitimately absent" and "has not uploaded yet" are the
same observation, and `windows-11-arm` is the slowest leg in the release. On
v2.36.0 the generator ran **40 minutes** before that `.exe` attached, printed
`not in this release; the Scoop and winget arm64 entries are dropped`, and
published ARM-less manifests. The asset then arrived, the in-repo copies were
regenerated by hand afterwards, and so the repository and the published channels
disagreed about ARM64 support instead of either one being visibly wrong.

The wait now waits for the producing workflow **runs** to finish first, which is
what tells the two cases apart: once `build-windows` has completed, an absent
arm64 `.exe` means the leg failed, which is the case the generator's fallback is
for. Their *conclusion* is deliberately not checked — requiring success would
let a failed optional leg block the channels that never needed it — and a tag
with no run at all (a dispatched publish of an older release) is not waited on.

`tests/test_release_publish_waits_for_the_builders.py` drives the wait's own
...
### Fixed — the Chocolatey package checksummed arm64 and downloaded x64

`render-chocolatey.py` built the download URL and the checksum independently: the URL
hardcoded `windows-x64.exe` while the checksum took the first `.exe` line in
`SHA256SUMS.txt`, which is arm64. Every `choco install` would have failed on a checksum
mismatch — the two facts about the same file came from two different places, and nothing
forced them to agree.

`exe_name(version)` is now the single source both the URL and the checksum derive from, so
naming two different files is no longer representable. Three tests come with it, each
failing against the previous code.

### Fixed — the Fedora COPR spec was pinned at 2.18.2, and the repository did not exist yet

Two separate defects on the same channel, closed together. `packaging/fedora/yazses.spec`
carried `Version: 2.18.2` since the day it was written, so a build from it would have served
every Fedora and RHEL user a release seventeen versions old. Bumped to 2.36.0 and verified
by building in a clean `fedora:41` container: `yazses-2.36.0-1.fc41.x86_64.rpm` builds,
installs, `/usr/bin/yazses` resolves, and `yazses doctor` runs. `rpmbuild` also warned
`bogus date in %changelog` on both entries — 2026-08-14 is a Friday, not a Thursday, and the
new entry claimed a Saturday for a Sunday; `tests/test_rpm_changelog_dates.py` now checks
weekday against date.

Separately, `docs/install-linux.md` had documented `dnf copr enable mskazemi/yazses` since
the spec was written, and the COPR repository itself did not exist — those three commands
could not have worked for anyone who tried them. It exists now, serving 2.36.0 for Fedora 43
and 44 on x86_64 and aarch64, and EPEL 10 on x86_64 (EPEL 9 is dropped rather than left red:
it ships Python 3.9 against a spec that needs 3.11). The build needed `--enable-net on` —
COPR's mock chroot has no network by default, and `%install` runs `pip` to populate the
bundled virtualenv — which a local container build never surfaces, since Docker and podman
give a build network access by default.

### Added — the Microsoft Store listing artwork was the retired logo; it is generated now

`packaging/store/boxart-1080.png` was the old blue speech-bubble mark, retired everywhere
else in v2.28.0, whose fix reached the snap's own icon on 2026-08-18. This asset was last
hand-written on 2026-08-14 by a `cairosvg` snippet pasted into a README and regenerated by
nothing since. Store art now renders from `brandmark.render_mark` via
`scripts/gen-store-art.py` (`make store-art`), with a drift test comparing pixels within a
measured tolerance rather than byte-for-byte — `render_mark` supersamples in floating point
and differs slightly across architectures. Also adds the 2:3 poster the Store recommends,
which did not exist.

### Added — YazSes can package as a full-trust MSIX, the free route to the Microsoft Store

The premise behind treating Store distribution as blocked on a paid code-signing certificate
was wrong: Store policy 10.2.9's certificate requirement applies to the EXE/MSI
direct-download path, and an MSIX is signed by Microsoft during certification instead.
Packaging as MSIX does not force the AppContainer sandbox either — that is opt-in via
`uap10:TrustLevel` — so the manifest declares `mediumIL` + `runFullTrust`, the same
integrity level the unpackaged `.exe` already runs at. A new `msix-validate` workflow runs
`makeappx` for real, asserts the built package is unsigned, and breaks a substitution to
prove the abort path aborts; the app identity stays a placeholder that `build-msix.ps1`
refuses to ship. This does not change the `.exe` on any other channel, which Store signing
never covered.

### Added — an opt-in Unicode injector backend types the characters ydotool silently drops

The existing Wayland `ydotool` path drops any character outside its keycode table, so
non-ASCII dictation — accented letters, most non-Latin scripts — is silently truncated or
mangled on Wayland with that backend selected. `inject/unicode_injector.py` is a fifth,
opt-in injector: it resolves each character through `libxkbcommon`'s XKB state and emits key
events via a private `/dev/uinput` keyboard, wired through the same config validation,
backend selection and `doctor` readiness as the existing four. It ships non-default: real
Wayland hardware validation of the actual character set is still pending, consistent with
how other experimental backends here have shipped.

Merging it against the Wayland portal backend landing in parallel surfaced a second,
pre-existing bug: `configcheck.py` never had `"portal"` in its accepted `[injection]
backend` values, so setting it was silently reset to `"auto"`. Fixed in the same merge.

Contributed by [@vortsghost2025](https://github.com/vortsghost2025)
([#364](https://github.com/MSKazemi/yazses/pull/364)), for
...
wheels, each with a URL and a SHA-256. Its whole value is that it pins. It is
generated by `uv pip compile` and `req2flatpak`, and those two were installed by

```
python3 -m pip install --quiet --upgrade uv req2flatpak
```

— unpinned, with `--upgrade` to be certain of taking whatever PyPI served that
minute. The program deciding what the pins *are* had no pin at all.

That inversion is worse than an ordinary unpinned CI tool because of the shape of
the output. A compromised `req2flatpak` writes attacker URLs with matching hashes
into a JSON file a human then reviews and commits — and the hash is exactly the
field a reviewer cannot check by reading. The result would be perfectly pinned, to
the wrong thing.

The generator's inputs are now hash-pinned in
`packaging/flatpak/tools-requirements.txt` and installed with `--require-hashes`,
so pip refuses an artifact whose digest does not match. The closure is four
packages, so it costs almost nothing to carry. `packaging==21.3` in that file is
not an oversight: `req2flatpak` 0.3.1 declares `packaging<22.0,>=21.3`, and
`pyparsing` is there only because packaging 21.x still needed it. The file records
the exact `uv pip compile` command that produced it, because pins go stale and the
person updating them will not be the person who wrote them — without the recipe the
likely next move is to delete the hashes rather than regenerate them.

Verified rather than assumed: the pinned set installs cleanly in a fresh venv and
`req2flatpak` runs; corrupting its hashes makes pip exit 1 with *"THESE PACKAGES DO
NOT MATCH THE HASHES FROM THE REQUIREMENTS FILE"*.

`tests/test_flatpak_generator_tools_are_pinned.py` holds it: the workflow must keep
`--require-hashes` and the committed file, the file must stay hash-complete, it must
pin the two tools the job actually runs, and it must not pass vacuously when the
file pins nothing. All four proved by sabotage.

Deliberately scoped to this one workflow. The other `pip install` lines under
`.github/workflows/` install test dependencies into a throwaway runner and produce
nothing shipped or committed; the FreeBSD leg installs by name on purpose. The line
is not "CI versus release" — it is whether the command produces a pinned artefact
