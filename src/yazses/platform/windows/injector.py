...
# hook can tell the text we type from the keys the user pressed. Without it, a
# command sequence containing Ctrl feeds the hook a modifier press and the
# daemon triggers itself — the self-capture the Linux backend avoids by
# refusing to listen on ydotool/uinput devices. Any non-zero constant works;
# this one is recognisable in a debugger.
...
class WindowsInjector:
    ...
    def inject_backspaces(self, count: int) -> None:
        ...
        for i in range(count):
            ...
            up.type = INPUT_KEYBOARD
            up.ki = _KEYBDINPUT(
                wVk=VK_BACK, wScan=0, dwFlags=KEYEVENTF_KEYUP, time=0, dwExtraInfo=INJECTED_TAG
            )
        sent = _load_user32().SendInput(len(inputs), inputs, ctypes.sizeof(_INPUT))
        if sent != len(inputs):
            log.warning(
    ...
    def inject_key_sequence(self, keys: list[str]) -> None:
        ...
        def _make_vk_event(vk: int, flags: int) -> _INPUT:
            ...
            inp.ki = _KEYBDINPUT(wVk=vk, wScan=0, dwFlags=flags, time=0, dwExtraInfo=INJECTED_TAG)
            return inp
